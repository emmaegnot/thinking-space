...
test("Checks page is rendered with the correct title and icon", async () => {
    const res = await request(app).get("/choose_word") // Make request to server
...
test("Checks nav contains title and home link", async () => {
    const res = await request(app).get("/choose_word");
...
test("Checks if the word selection buttons are present", async () => {
    ...
    emotions.forEach(emotion => {
        expect(res.text).toContain(`<button type=\"button\" name=\"emotion\" value=\"${emotion}\" onclick=\"highlightButton(this), showimage1()`); // For each emotion, check the response contains an input field with its name
    });
});
